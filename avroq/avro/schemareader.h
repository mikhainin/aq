...
private:
    ...
    std::unique_ptr<node::Node> parseOneJsonObject(const boost::property_tree::ptree &node);
    std::unique_ptr<NodeDescriptor> descriptorForJsonObject(const boost::property_tree::ptree &node);
    std::unique_ptr<node::Node> nodeByTypeName(const std::string &typeName, const std::string &itemName);
    ...
    std::unique_ptr<node::Node> nodeByType(const boost::property_tree::ptree &node,
